...
class GStreamerPluginsBadConan(ConanFile):
    ...
    def build_requirements(self):
        ...
        if self.settings.arch == "x86_64" and (
            self.options.nvenc or self.options.nvdec
        ):
    ...
    def source(self):
        ...
        if self.options.aiveropatchlatency:
            tools.patch(
                patch_file="reduce_latency.patch",
                base_path=os.path.join(self.source_folder, "gst-plugins-bad"),
            )

    def build(self):
        args = ["--auto-features=disabled"]
        args.append(
            "-Dvideoparsers=" + ("enabled" if self.options.videoparsers else "disabled")
        )
        ...
        args.append(
            "-Dmpegtsmux=" + ("enabled" if self.options.mpegtsmux else "disabled")
        )
        args.append(
            "-Dmpegtsdemux=" + ("enabled" if self.options.mpegtsdemux else "disabled")
        )
        args.append(
            "-Ddebugutils=" + ("enabled" if self.options.debugutils else "disabled")
        )
        ...
        if self.settings.arch == "x86_64":
            ...
            if self.version == "master":
                args.append(
                    "-Dnvcodec=" + ("enabled" if self.options.nvcodec else "disabled")
                )
