...
class KinectAzureSensorSDKConan(ConanFile):
    ...
    def source(self):
        ...
        tools.download(
            "%s/libk4a%s/%s" % (debian_repo_url, version_short, libk4a), filename=libk4a
        )
        tools.download(
            "%s/libk4a%s-dev/%s" % (debian_repo_url, version_short, libk4a_dev),
            filename=libk4a_dev,
        )
    ...
    def package(self):
        ...
        self.copy(
            "*", src="libk4a/usr/lib/" + lib_dir_arch[0], dst="lib", symlinks=True
        )
