...
This example requires xref:system:ROOT:buildingblocks:page:gstreamer$b-rgbd.adoc[`rgbddemux`] in order to demultiplex a single `video/rgbd` stream into multiple `video/x-raw` streams.

[source,sourceCode,bash]
----
export RS_SERIAL=XXXXXXXXXXXX

gst-launch-1.0 rgbddemux name=rsdemux \
realsensesrc serial=${RS_SERIAL} enable-depth=true enable-infra1=true enable-infra2=true enable-color=true ! \
rsdemux.sink \
rsdemux.src_depth ! queue ! glimagesink  \
rsdemux.src_infra1 ! queue ! glimagesink \
rsdemux.src_infra2 ! queue ! glimagesink \
rsdemux.src_color ! queue ! glimagesink 
----
...
The buffers for all *auxiliary streams* are attached to the GstBuffer for the *main stream* by the use of `BufferMeta` described in xref:system:ROOT:buildingblocks:page$a-depth-meta.adoc[Depth Meta]. Furthermore, all buffers, both for *main stream* and *auxiliary streams*, are tagged using the `TagsMeta`.
...
All GstBuffers for *main stream* and all *auxiliary streams* are explicitly given `duration` as well as `pts` and `dts` timestamps. You can set the timestamping mode via `timestamp-mode` property, please see xref:system:ROOT:concepts:page$e-timestamping-and-synchronisation.adoc[Timestamping and Synchronisation] for more details. 
...
You can enable `attach-camera-meta` to stream meta associated with RS device such as intrinsics, extrinsics and depth scale. Please see xref:system:ROOT:concepts:page$g-camera-meta.adoc[Camera Meta] for more details.
...
We use Cap’n Proto for serialisation, please see xref:system:ROOT:concepts:page:f-storage-container-format$c-capnproto.adoc[Metadata format and handling] for more information. When the `include-per-frame-metadata` property is set to true, the `realsensesrc` attaches the Cap’n Proto serialized metadata buffers to the buffer of *main stream* using the `BufferMeta` API. These buffers are tagged, using the `TagsMeta` API as `%smeta`, where `%s` is the tag of the stream, e.g. _depth_ or _infra1_.
